...
class amazonbookSpider(CommonSpider):
    ...
    xpath_rules = {
        "//div[@class='inner']/div[@class='a-row']": {
            "url": ".//a[@class='title ntTitle noLinkDecoration']/@href",
            "title": ".//span[@class='s9TitleText']/text()",
            "comments": ".//span[@class='a-size-small']/text()",
        }
    }

    def parse_0(self, response):
        info("Parse 0 " + response.url)
        pp.pprint(self.parse_with_rules_xpath(response, self.xpath_rules, dict))
