...
def unpickle(file_):
    with open(os.path.join(DATA_PATH, file_), 'rb') as fo:
...
def build_second_net():
    ...
    conv3_float = tf.reshape(conv3_pool, [-1, C3])
    conv3_drop = tf.nn.dropout(conv3_float, keep_prob=keep_prob)
...
2017-12-05 14:30:06.544606: I C:\tf_jenkins\home\workspace\rel-win\M\windows\PY\35\tensorflow\core\platform\cpu_feature_guard.cc:137] Your CPU supports instructions that this TensorFlow binary was not compiled to use: AVX AVX2
Accuracy: 10.0%
Accuracy: 42.28%
Accuracy: 45.71%
Accuracy: 51.46%
