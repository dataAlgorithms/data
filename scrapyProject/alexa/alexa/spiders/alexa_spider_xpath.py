...
class alexaSpider(CrawlSpider):
    ...
    rules = [
        Rule(sle(allow=("/topsites/category;?[0-9]*/Top/[^/]*$")), callback='parse_category_top_xxx_xpath', follow=True),
        Rule(sle(allow=("/topsites/category/Top$", )), callback='parse_category_top_xpath', follow=True),
    ...
    def parse_category_top_xxx_xpath(self, response):
        info('parsed ' + str(response))
        items = []
        sel = Selector(response)

        sites = sel.xpath('//li[@class="site-listing"]')
        for site in sites:
            item = alexaSiteInfoItem()
            item['url'] = site.xpath('.//a[contains(@href, "siteinfo")]/@href')[0].extract()
            item['name'] = site.xpath('.//a[contains(@href, "siteinfo")]/text()')[0].extract()
            item['description'] = site.xpath('.//div[@class="description"]/text()')[0].extract()
            remainder = site.xpath('.//span[@class="remainder"]/text()')
            if remainder:
    ...
    def parse_category_top_xpath(self, response):
        info('parsed ' + str(response))
        items = []
        sel = Selector(response)
        categories = sel.xpath('.//li/a[contains(@href, "/topsites/category/Top/")]')
        for category in categories:
            item = alexaCategoryItem()
            item['url'] = category.xpath('@href')[0].extract()
            item['name'] = category.xpath('text()')[0].extract()
            items.append(item)
        return items
